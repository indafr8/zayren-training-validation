...
def names_models_routes(df_names_models, route_s2s):
    ...
    scalerx_name = df_names_models.loc[df_names_models['s2s_route'] == route_s2s, 'scaler_X'].values[0]
    scalery_name = df_names_models.loc[df_names_models['s2s_route'] == route_s2s, 'scaler_y'].values[0]
...
def names_models_distance(df_names_models, type_model):
    ...
    scalerx_name = df_names_models.loc[df_names_models['models'] == type_model, 'scaler_X'].values[0]
    scalery_name = df_names_models.loc[df_names_models['models'] == type_model, 'scaler_y'].values[0]

    return lr_model_name, svr_model_name, rnn_model_name, scalerx_name, scalery_name

def load_models(folder_path, lr_model_name, svr_model_name, rnn_model_name, scalerx_name, scalery_name, new_model_name):
    #load models
...
def ml_distance_prediction(model, distance,month):
    X_pred = np.array([[distance,month]])  # <-- Corrección aquí
...
def rnn_prediction(model, scaler_X, scaler_y, distance, month, id):
    ...
    X_pred = X_pred.reshape((X_pred.shape[0], 1, X_pred.shape[1]))  # Corrección
    prediction = model.predict(X_pred)
...
def price_pred(broker_id, route_s2s, query, distance, month):
    if query == 'city2city' or query == 'state2state':
        ...
        rnn_pred = rnn_distance_prediction(rnn_model, scaler_X, scaler_y, distance, month)  # ← Cambio aquí

        #print(f"LR:{lr_pred}, SVR:{svr_pred}, RNN: {rnn_pred}")
        price = ensemble(svr_pred, lr_pred, rnn_pred)
        
        return lr_pred, svr_pred, rnn_pred, price
